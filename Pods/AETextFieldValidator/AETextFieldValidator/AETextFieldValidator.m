...
-(BOOL)validate{
    ...
    for (int i=0; i<[arrRegx count]; i++) {
        ...
        if([dic objectForKey:@"confirm"] && [self.placeholder containsString:@"Confirm Password"]){
    ...
    if (self.restorationIdentifier) {
        ...
        if([self.text length]!=0 && ![self validateString:self.text withRegex:self.restorationIdentifier] && ![self.restorationIdentifier isEqualToString:@""]){
            
            [self showErrorIconForMsg:[self.restorationIdentifier componentsSeparatedByString:@"|"][1]];
            return NO;
        }
    }
...
- (BOOL)validateString:(NSString*)stringToSearch withRegex:(NSString*)regexString {
    ...
    NSPredicate *regex = [NSPredicate predicateWithFormat:@"SELF MATCHES %@",[self.restorationIdentifier componentsSeparatedByString:@"|"][0]];
