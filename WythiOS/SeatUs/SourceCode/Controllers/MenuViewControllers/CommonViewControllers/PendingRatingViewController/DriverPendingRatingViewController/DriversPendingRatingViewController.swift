//
//  DriversPendingRatingViewController.swift
//  SeatUs
//
...
class DriversPendingRatingViewController: BaseViewController {
    ...
    @IBAction func onSaveClicked(_ sender: Any) {
        ...
        if isAllRated {
            ...
            Trip.rateTrip(rateArray: rateArray, isDriver: true)
...
extension DriversPendingRatingViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.delegate = self
        cell.initializeCell(contentArray[indexPath.row].driver, ratings: ratings, trip: contentArray[indexPath.row].trip)
        return cell
    }
...
extension DriversPendingRatingViewController: PassengerRatingCellDelegate {
    func onRateNowClicked(_ sender: Any, tag: Int) {
        ...
        cont.name = (contentArray[tag].driver.first_name ?? "") + " " + (contentArray[tag].driver.last_name ?? "")
        cont.show(controller: self.parent!)
        cont.selectButtonTapped = { selectedData in
            let user_id = self.contentArray[tag].driver.user_id?.stringValue
